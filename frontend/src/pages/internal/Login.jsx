  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      ...
      if (user.role === Role.INTERNAL) {
        ...
        setError('Access denied. Internal user credentials required.');
      ...
      <div className="w-full max-w-md">
        <div className="bg-card border border-border rounded-lg shadow-sm p-8">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-semibold text-foreground">Internal Login</h1>
            <p className="text-sm text-muted-foreground mt-2">
              Sign in to access internal operations
            </p>
          </div>
          ...
          <div className="mt-6 text-center text-sm">
            <Link to="/internal/reset-password" className="text-primary hover:underline">
              Forgot password?
            </Link>
          </div>
        </div>
